...
def process():
    ...
    parser = argparse.ArgumentParser(prog='Deep Learning Causality', description='Investigate the Granger causality using linear and non-linear measures.')
    parser.add_argument('--data', nargs=1, type=str,  help='Specify the input data distribution.')
    ...
    parser.add_argument('--beta', nargs=1, type=int, help='Specify the value of time-lag.')
    parser.add_argument('--num_tap',  nargs='+', type=str,  help='The number of taps.')
    parser.add_argument('--walk_time',  nargs='+', type=str, help='The time in seconds for a person to walk to the tap.')
    parser.add_argument('--flow_rate', nargs='+', type=str, help='Flow Rate (per second) for each tap.')
